...
export async function deployAddon(projectPath: string, projectName: string): Promise<DeployResult> {
  // 1. verifyBuild
...
async function pollDeployment(client: VercelClient, deployId: string): Promise<{ status: string; url?: string }> {
...
export async function verifyBuild(projectPath: string): Promise<{ success: boolean; error?: string }> {
  try {
    // npm install
